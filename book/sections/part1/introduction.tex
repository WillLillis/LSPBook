...
Without going into too much detail, the \texttt{.data} directive simply tells the assembler to assemble the following statements into a data subsection. Underneath that, \texttt{.equ} works essentially the same as the C preprocessor's \texttt{\#define}. Finally \texttt{.asciz} creates a null terminated C string. We then grab the length of the message. Some data gets moved around so we can make the \texttt{write} and \texttt{exit} system calls, and that's about it! The specifics can vary depending on architecture and assembler you're using, but most assembly is fairly similar in structure. 
