...
Let's take advantage of this neat Markdown integration! All we really need to do is supply a \texttt{String} containing our Markdown content. Adding the appropriate code to our event loop, we can again use the types provided by \texttt{lsp\_types} to create a \texttt{Hover} response. This struct is then serialized to transmit via RPC as our response object with our call to \texttt{serde\_json::to\_value()}. After that, we send it off! 
...
Now that we've taken care of the \texttt{Display} trait, displaying our documentation is as simple as adding a single call to the \texttt{format!()} macro...
...
% try it out, screenshots, encourage reader to try out different formatting, etc.

% Fix bug with missing associated names
